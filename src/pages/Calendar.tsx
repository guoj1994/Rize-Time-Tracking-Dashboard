...
import { CalendarIcon, ChevronLeftIcon, ChevronRightIcon, SparklesIcon } from 'lucide-react';
...
export function Calendar() {
  const { state, sessionKind, sessionSeconds, categoryFor, isOverridden, setCategory } = useTracking();
  ...
  const isLive = state === 'running' || state === 'break';
  const liveCategory: ActivityCategory = state === 'break' ? 'break' : sessionKind === 'meeting' ? 'meeting' : 'focus';
        ...
        <aside className="rize-scroll w-[320px] shrink-0 overflow-y-auto border-l border-line bg-surface">
          ...
          <div className="p-4">
            ...
            <p className="mt-3 text-[12px] text-faint">
                  You corrected this one — the classifier uses it as an example for similar activity.
                </p>
            }
            </div> :
              ...
              <ul className="mt-2 space-y-2">
                ...
                <li key={block.id} className="rounded-lg border border-line p-2.5">
                        ...
                        <p className="mt-0.5 text-[12px] text-faint">
                          Guessed {categoryMeta[block.aiCategory].label} ·{' '}
